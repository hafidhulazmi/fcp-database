...
func (u *sessionsRepoImpl) AddSessions(session model.Session) error {
	_, err := u.db.Exec("INSERT INTO sessions (username, token, expiry) VALUES ($1, $2, $3)", session.Username, session.Token, session.Expiry)
	if err != nil {
		return err
	}
...
func (u *sessionsRepoImpl) SessionAvailName(name string) error {
	row := u.db.QueryRow("SELECT token FROM sessions WHERE username = $1", name)
